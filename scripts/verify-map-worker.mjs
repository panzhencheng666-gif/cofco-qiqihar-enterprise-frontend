...
for (const worker of workers) {
  assert.ok(
    scripts.some((script) => script.includes(`/overview-monitoring/assets/${worker}`)),
    "Map renderer must reference the emitted worker under the deployment base",
  );
  assert.ok(
    (await readFile(path.join(assets, worker), "utf8")).length > 10000,
    "Worker must contain executable map processing code",
  );
}
