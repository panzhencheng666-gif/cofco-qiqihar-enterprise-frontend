...
it("checks immediately when the network recovers and removes listeners on stop", async () => {
  vi.useFakeTimers();
  const read = vi.fn().mockResolvedValueOnce("a").mockResolvedValue("b");
  const changed = vi.fn();
  const stop = new BrowserOverviewMapRevisionMonitor(read).subscribe(changed);
  await vi.advanceTimersByTimeAsync(0);
...
it("discards a late revision after unsubscribe", async () => {
  let finish!: (s: string) => void;
  const changed = vi.fn();
  const stop = new BrowserOverviewMapRevisionMonitor(
    () =>
