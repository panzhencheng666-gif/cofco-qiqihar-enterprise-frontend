...
const row = {
  ...
  types: [{ code: "PROCESSOR", name: "加工企业", iconKey: "market" }],
  longitude: 123.4,
  latitude: 48.2,
...
describe("thin map catalogue", () => {
  it("coalesces a region load and filters the same response without fetching observations or full master pages", async () => {
    ...
    const [a, b] = await Promise.all([
      r.mapCatalog(scope),
      r.mapCatalog({ ...scope, categoryCode: "MARKET" }),
    ]);
    expect(get).toHaveBeenCalledTimes(1);
    expect(get.mock.calls[0]?.[0]).toContain("/overview/map-samples?");
    expect(a.list.totalCount).toBe(1);
    expect(a.icons.length).toBe(a.list.totalCount);
    expect(b.list.items).toHaveLength(1);
    expect((await r.mapCatalog({ ...scope, query: "不存在" })).icons).toHaveLength(0);
    expect(get).toHaveBeenCalledTimes(1);
    r.invalidateFormalCatalog();
    get.mockResolvedValue({ data: [] });
    expect((await r.mapCatalog(scope)).list.totalCount).toBe(0);
