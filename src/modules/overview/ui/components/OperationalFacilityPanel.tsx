...
function StoragePanel({
  ...
  return (
    <div className="operational-facility-panel is-storage">
      ...
      {selected ? (
        <StorageFacilityCard facility={selected} />
      ) : (
        <p>当前范围没有已核验的关联库点。</p>
      )}
...
export function StorageFacilityCard({
  ...
  return (
    <article className="storage-facility-card" aria-label={`${selected.name}库点详情`}>
        ...
        {selected.prices.length ? (
          selected.prices.map((price) => (
            ...
            >
              ...
              <p>
                {price.productName ?? price.productCode} · {price.qualityRequirement}
              </p>
              <small>
                {price.effectiveOn} ·{" "}
...
export function RailwayFacilityCard({
  catalogue,
  facility: selected,
}: {
  catalogue: OperationalFacilityCatalogue;
  facility: RailwayFacility;
}) {
  return (
    <article
          ...
          {catalogue.railwayLines.length ? (
            catalogue.railwayLines.map((line) => (
              <li key={line.name}>
                <b>{line.name}</b>
                <span>
                  {line.mappedTrackKm.toFixed(1)} 公里 · {line.usage || "用途未标注"}
                </span>
