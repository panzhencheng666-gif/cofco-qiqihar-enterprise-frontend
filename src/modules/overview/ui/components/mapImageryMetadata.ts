...
const RELEASE_VERSION = /^\d{4}-W\d{2}$/;
...
export function imageryLabel(metadata: MapImageryMetadata): string {
  ...
  const acquisition =
    metadata.acquisitionFrom && metadata.acquisitionTo
      ...
      : " · 历史影像";
  const cadence = metadata.updateCadence === "WEEKLY" ? " · 每周一同步" : "";
  return `${metadata.provider}${resolution}${acquisition}${cadence}`;
}

export function imageryWarning(metadata: MapImageryMetadata): string | undefined {
  if (metadata.status === "STALE") return "本周同步未成功，当前沿用上一成功版本。";
  if (metadata.status === "FALLBACK") return "当前显示历史影像，不代表本周地表现状。";
  return undefined;
}
