...
describe("WorkItemsPage", () => {
  it("renders pending status tabs from the backend definition and runs real queries", async () => {
    ...
    expect(await screen.findByRole("button", { name: "全部" })).toBeVisible();
    expect(screen.getByRole("button", { name: "待填报" })).toBeVisible();
    expect(screen.getByRole("button", { name: "待审核" })).toBeVisible();
    expect(screen.getByRole("button", { name: "退回补充" })).toBeVisible();
    expect(screen.getByRole("button", { name: "异常处理" })).toBeVisible();
